...
(
    ...
    CONSTRAINT chk_recurring_expenses_amount
        CHECK (amount > 0),

    CONSTRAINT chk_recurring_expenses_dates
        CHECK (
            end_date IS NULL
                OR end_date >= next_due_date
            ),

    CONSTRAINT chk_recurring_expenses_frequency
        CHECK (
            frequency IN (
                          'WEEKLY',
                          'MONTHLY',
...
CREATE INDEX idx_recurring_expenses_user_id
...
CREATE INDEX idx_recurring_expenses_category_id
